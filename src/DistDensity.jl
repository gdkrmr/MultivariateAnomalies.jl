...
function dist_matrix!(D_out::Tuple{Array{tp,2},Array{tp,2},Array{tp,2}}, data::AbstractArray{tp, N}; dist::String = "Euclidean", space::Int = 0, lat::Int = 0, lon::Int = 0, Q = 0, dims = 2) where {tp, N}
  ...
  transpose!(tdat, dat)
  if(dist == "Euclidean")         Distances.pairwise!(Euclidean(),      D, tdat, dims = dims)
  elseif(dist == "SqEuclidean")   Distances.pairwise!(SqEuclidean(),    D, tdat, dims = dims)
  elseif(dist == "Chebyshev")     Distances.pairwise!(Chebyshev(),      D, tdat, dims = dims)
  elseif(dist == "Cityblock")     Distances.pairwise!(Cityblock(),      D, tdat, dims = dims)
  elseif(dist == "JSDivergence")  Distances.pairwise!(JSDivergence(),   D, tdat, dims = dims)
  elseif(dist == "Mahalanobis")   Distances.pairwise!(Mahalanobis(Q),   D, tdat, dims = dims)
  elseif(dist == "SqMahalanobis") Distances.pairwise!(SqMahalanobis(Q), D, tdat, dims = dims)
...
function dist_matrix!(D_out::Tuple{Array{tp,2},Array{tp,2},Array{tp,2},Array{tp,2},Array{tp,2}},
  ...
  if(dist == "Euclidean")         Distances.pairwise!(Euclidean(),      D, ttraindat, tdat, dims = dims)
  elseif(dist == "SqEuclidean")   Distances.pairwise!(SqEuclidean(),    D, ttraindat, tdat, dims = dims)
  elseif(dist == "Chebyshev")     Distances.pairwise!(Chebyshev(),      D, ttraindat, tdat, dims = dims)
  elseif(dist == "Cityblock")     Distances.pairwise!(Cityblock(),      D, ttraindat, tdat, dims = dims)
  elseif(dist == "JSDivergence")  Distances.pairwise!(JSDivergence(),   D, ttraindat, tdat, dims = dims)
  elseif(dist == "Mahalanobis")   Distances.pairwise!(Mahalanobis(Q),   D, ttraindat, tdat, dims = dims)
  elseif(dist == "SqMahalanobis") Distances.pairwise!(SqMahalanobis(Q), D, ttraindat, tdat, dims = dims)
  else print("$dist is not a defined distance metric, has to be one of 'Euclidean', 'SqEuclidean', 'Chebyshev', 'Cityblock' or 'JSDivergence'")
  end
  return(D_out[1])
end
